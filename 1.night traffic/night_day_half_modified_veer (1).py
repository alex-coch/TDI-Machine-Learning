...
distance = int(input(
    'Enter distance between 2 lines in Meters(for better results use 10 Meters): '))
...
def send(img):
    ...
    response = requests.post(api, data=data, timeout=5, headers={
...
# midtrack is last line Y pt and starttrack is first line Y pt. First line from TOP.
def max_images(speed_limit, fps, distance, midtrack, starttrack):
...
while True:
    ...
    if score > 200.0:  # DAY TIME
        ...
        # for improving the brightness and illumination
        ...
        cv2.line(frame_og, (l1_x1, int((lasttrack))),
                 (l1_x2, int((lasttrack))), (0, 0, 0), 1)
        cv2.line(frame_og, (l1_x1, pt3_pxl),
                 (l1_x2, pt3_pxl), (125, 200, 127), 3)

        if ret == True:
            foregroundMask = bgSubtractor.apply(frame)
            foregroundMask = cv2.morphologyEx(
                foregroundMask, cv2.MORPH_OPEN, kernel)
            foregroundMask = cv2.erode(foregroundMask, kernel, iterations=3)
            foregroundMask = cv2.morphologyEx(
                foregroundMask, cv2.MORPH_CLOSE, kernel, iterations=6)
            foregroundMask = cv2.dilate(
                foregroundMask, kernel_di, iterations=7)
            foregroundMask = cv2.medianBlur(foregroundMask, 5)
            thresh = cv2.threshold(
                foregroundMask, 25, 255, cv2.THRESH_BINARY)[1]
                ...
                if cy > midtrack and cy < pt3_pxl:
                    ...
                    if seconds <= 0.2:
                        ...
                        if flag is False:
                            ...
                            cv2.putText(frame_og, str(int(speed)), (x, y),
                                        font, 2, (255, 255, 255), 4, cv2.LINE_AA)
                            cv2.putText(frame, str(int(speed)), (x, y),
                                        font, 2, (255, 255, 255), 4, cv2.LINE_AA)
                        ...
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(frame, str(int(speed)), (x, y),
                                    font, 2, (255, 255, 255), 8, cv2.LINE_AA)
            ...
            for contour1, hier1 in zip(contours1, hierarchy1):
                ...
                cy1 = int((h1 / 2) + y1)
                if w1 > 10 and h1 > 10:
                    cv2.rectangle(frame_og, (x1 - 10, y1 - 10),
                                  (x1 + w1, y1 + h1), (255, 255, 0), 2)
            ...
            if cy1 > starttrack and w1 > 10 and h1 > 10:
                ...
                if cy1 > midtrack and cy1 < pt3_pxl:
                    ...
                    if seconds1 <= 0.2:
                        ...
                        if flag1 is False:
                            font = cv2.FONT_HERSHEY_SIMPLEX
                            cv2.putText(frame_og, str(
                                int(speed1)), (x1, y1), font, 2, (255, 255, 255), 8, cv2.LINE_AA)
                            cv2.putText(frame, str(int(speed1)), (x1, y1),
                                        font, 2, (255, 255, 255), 8, cv2.LINE_AA)
                            # if not os.path.exists(path):
                            #     os.makedirs(path)
                        ...
                        flag1 = True
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(frame_og, str(int(speed1)), (x1, y1),
                                    font, 2, (255, 255, 255), 8, cv2.LINE_AA)
    ...
    #     break
    ...
    elif cy2 > new__start_y:  # NIGHT TIME
        if flag2 is True and cy2 < new_mid_y:
            ...
            speed1 = (distance/frame_crossed3)*framespersecond * \
                      3.6  # COSINE CORRECTION############## '''takes an angle of camera'''
            Angle = math.radians(Angle)
            Angle = math.cos(Angle)
			...
			if int(avg_speed) > speed_limit and cy2 > lasttrack:
				cv2.imshow("Lane_1", roi)  # roi = frame[y-50:y + h, x:x + w]
				lane_1_1.append(roi)
